...
class FLREMOTECLIENT(object):
    ...
    def refreshQuery(self, curname, fields, table, where, cursor, conn):
        self.send_to_server(self.create_dict("refreshQuery", {"cursor_id": cursor.id_, "curname": curname, "fields": fields, "table": table, "where": where}))
        #print("*", curname)
        #print("*", fields)
        #print("*", table)
        #print("where", where)
    ...
    def refreshFetch(self, number, curname, table, cursor, fields, where_filter):
        self.send_to_server(self.create_dict("refreshFetch", {"cursor_id": cursor.id_, "curname": curname, "fields": fields, "table": table, "where_filter": where_filter, "number": number}))
        
    def fetchAll(self, cursor, tablename, where_filter, fields, curname):
        return self.send_to_server(self.create_dict("fetchAll", {"cursor_id": cursor.id_, "tablename": tablename, "where_filter": where_filter, "fields": fields , "curname": curname}))

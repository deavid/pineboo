...
def process_error(error_str: str) -> str:
    ...
    if "AttributeError: 'dict' object has no attribute" in error_str:
        ...
        var = var.replace("\n", "")
        ret += translate("scripts", "La forma correcta de acceder a .%s es [%s].") % (var, var)
        ...
        var = var.replace("\n", "")
        var = var.replace("'", "")
        ret += translate("scripts", "No se ha traducido el script o el script está vacio.")
    elif "object is not callable" in error_str:
        error = "object is not callable"
        var = error_str[error_str.find("TypeError") + 10 : error_str.find(error)]
        ret += translate("scripts", "Estas llamando a un objeto %s .Los parentesis finales hay que quitarlos." % var)

...
class XMLAction(XMLStruct):
    ...
    def loadRecord(self, cursor: Optional["PNSqlCursor"]) -> "FLFormRecordDB":
        ...
        if not self._loaded:
            if self.formrecord_widget and getattr(
    ...
    def load(self) -> "FLFormDB":
        ...
        if not self._loaded:
            ...
            if self.project.DGI.useDesktop() and hasattr(
                self.project.main_window, "w_"
            ):
                ...
                self.logger.info(
                    "Loading action %s (load_script %s). . . ",
                    self.name,
                    self.scriptform,
                )
                ...
                self.mainform_widget = (
                    script.form
                )  # FormDBWidget FIXME: Add interface for types
    ...
    def load_script(
        ...
        script_loaded.form = script_loaded.FormInternalObj(
            action_, self.project, parent_object
        )

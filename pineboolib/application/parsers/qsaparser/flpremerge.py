            ...
            if desde is None or desde >= anthasta + 1:
                ...
                for line in sB.splitlines(True):
                    ...
                    if isseparator:
                        ...
                        ltype = "comment_inline"
                    elif iscommentline2:
                        ltype = "comment_block_inline"
                    elif iscommentbegin:
                        ltype = "comment_block"
                    elif iscommentend:
                        ltype = "comment_blockend"
...
class FindEquivalences(object):
    ...
    def getMaxKnown(self, pkA):
        ...
        if len(pkA) == 0:
            return 1.0, None
        if pkA not in self.max_known_eq:
            return 0.0, None
...
def isinside(parent, child):
    ...
    cfrom, cto = child
    pto += 1
    if cfrom >= pfrom and cto <= pto:
        return 0
    if cfrom < pfrom and cto < pfrom:
        return -1
    if cfrom > pto and cto > pto:
        return 1

...
class FLManager(QtCore.QObject, IManager):
                ...
                if (
                    ...
                    if must_alter:
                        if not self.alterTable(stream, stream, None, True):
                            logger.warning(
                                "La regeneración de la tabla %s ha fallado", n
                            )
                ...
                if elem_field is not None and elem_level is not None:
                    if elem_level.text is not None and elem_field.text is not None:
                        if float(elem_level.text.strip(" \t\n\r")) == i:
                            ...
                            q.addGroup(
                                PNGroupByQuery(i, elem_field.text.strip(" \t\n\r"))
                            )
        ...
        while not no.isNull():
            ...
            if not e.isNull():
                if e.tagName() == "action":
                    ...
                    if nl.count() == 0:
                        self.logger.warning(
                            "Debe indicar la etiqueta <name> en acción '%s'" % n
                        )
                        no = no.nextSibling()
                        continue
                    else:
                    ...
                    if is_valid_name:
                        if not e2.isNull():
                            ...
                            self.logger.debug(
                                "WARN: Se encontró una acción vacia para '%s'." % n
                            )
    ...
    def createTable(self, n_or_tmd) -> Any:
        ...
        if isinstance(n_or_tmd, str):
            ...
            if not self.db_.createTable(n_or_tmd):
                logger.warning(
                    "FLManager : %s",
                    util.tr("No se ha podido crear la tabla ") + n_or_tmd.name(),
                )
                return False
        ...
        if isinstance(args[0], PNFieldMetaData):
            ...
            if not mtd:
                return self.formatAssignValueLike(
                    args[0].name(), args[0].type(), args[1], args[2]
                )
    ...
    def formatAssignValue(self, *args, **kwargs) -> str:
        ...
        if isinstance(args[0], PNFieldMetaData) and len(args) == 3:
            ...
            if fMD.isPrimaryKey():
                return self.formatAssignValue(
                    mtd.primaryKey(True), fMD.type(), args[1], args[2]
                )
        ...
        while q2.next():
            ...
            if not tmd:
                logger.warning(
                    ...
                    FLUtil().translate(
                        "application",
                        "No se ha podido crear los metadatatos para la tabla %s",
                    )
                    % table,
                )
    ...
    def storeLargeValue(self, mtd, largeValue: str) -> Optional[str]:
        ...
        if q.exec_() and q.first():
            ...
            if not util.execSql(sql, "Aux"):
                logger.warning(
                    "FLManager::ERROR:StoreLargeValue.Insert %s.%s", tableLarge, refKey
                )
                return None

...
class oficial extends interna
{
  ...
  function formatoNumeroSS340(numero, enteros, decimales)
  {
    return this.ctx.oficial_formatoNumeroSS340(numero, enteros, decimales);
  }
}
//// OFICIAL /////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////
...
function oficial_toolButtonAeat_clicked()
{
  ...
  if (parseInt(anoEjercicio) >= 2014) {
    _i.presTelematica2014();
  } else if (parseInt(anoEjercicio) >= 2012) {
...
function oficial_presTelematica2014()
{
  ...
  */
  ...
  while (qryFacturasEmitidas.next()) {
    ...
    debug("tama antes de fecha de cobro: " + contenidoEmitidas.length);
    nombreDato = util.translate("scripts", "Fecha de cobro");
    if ((contenidoEmitidas.length + 1) !=  445) {
      return this.iface.errorAcumuladoControl(445, nombreDato);
    }
    temp = qryFacturasEmitidas.value("fechacobro");
    temp = this.iface.formatoFecha(temp.toString());
    temp = flfactppal.iface.pub_espaciosDerecha(temp, 8);
    contenidoEmitidas += temp;
    contenidoCSV += "|" + temp;

    debug("Fecha de cobro:" + temp + "fin fecha de cobro");
    debug("tama antes de importe cobrado: " + contenidoEmitidas.length);
    nombreDato = util.translate("scripts", "Importe cobrado");
    if ((contenidoEmitidas.length + 1) !=  453) {
      return this.iface.errorAcumuladoControl(453, nombreDato);
    }
    temp = qryFacturasEmitidas.value("importecobrado");
    contenidoEmitidas += this.iface.formatoNumeroSS340(temp, 11, 2);
    contenidoCSV += "|" + this.iface.formatoNumeroCSV(temp, "co_facturasrec340", "importecobrado");
    debug("Importe cobrado:" + this.iface.formatoNumeroSS340(temp, 11, 2) + "fin importe cobrado");

    debug("tama antes de medio de cobro: " + contenidoEmitidas.length);
    nombreDato = util.translate("scripts", "Medio de cobro utilizado");
    if ((contenidoEmitidas.length + 1) !=  466) {
      return this.iface.errorAcumuladoControl(466, nombreDato);
    }
    temp = qryFacturasEmitidas.value("mediocobro");   
		temp = flfactppal.iface.pub_espaciosDerecha(temp, 1);
		
    contenidoEmitidas += temp;
    contenidoCSV += "|" + temp;
    debug("Medio de cobro:" + temp+"finmediocobro");
    debug("tama antes de cuenta bancaria: " + contenidoEmitidas.length);
    nombreDato = util.translate("scripts", "Cuenta bancaria o medio de cobro utilizado");
    if ((contenidoEmitidas.length + 1) !=  467) {
      return this.iface.errorAcumuladoControl(467, nombreDato);
    }
    temp = qryFacturasEmitidas.value("cuentabancaria");
    temp = flfactppal.iface.pub_espaciosDerecha(temp, 34);
    contenidoEmitidas += temp;
    contenidoCSV += "|" + temp;
    debug("Cuenta bancaria:" + temp +"fincuentabancaria");
  ...
  qryFacturasRecibidas.setSelect("cifnif, cifnifrp, apellidosnomrs, codpais, claveidentificacion, numidentificacion, tipolibro, operacion, fechaexpedicion, fechaoperacion, tipoimpositivo, baseimponible, cuotaimpuesto, importetotal, baseimponiblecoste, idenfactura, numregistro, numfacturas, desgloseregistro, intervidentif, intervidentif2, cuotadeducible, fechapago, importepagado, mediopago, cuentabancaria");
  ...
  while (qryFacturasRecibidas.next()) {
    ...
    nombreDato = util.translate("scripts", "Fecha de pago");
    if ((contenidoRecibidas.length + 1) !=  350) {
      return this.iface.errorAcumuladoControl(350, nombreDato);
    }
    temp = qryFacturasRecibidas.value("fechapago");
    temp = this.iface.formatoFecha(temp.toString());
    temp = flfactppal.iface.pub_espaciosDerecha(temp, 8);
    contenidoRecibidas += temp;
    contenidoCSV += "|" + temp;

    nombreDato = util.translate("scripts", "Importe pagado");
    if ((contenidoRecibidas.length + 1) !=  358) {
      return this.iface.errorAcumuladoControl(358, nombreDato);
    }
    temp = qryFacturasRecibidas.value("importepagado");
    contenidoRecibidas += this.iface.formatoNumeroSS340(temp, 11, 2);
    contenidoCSV += "|" + this.iface.formatoNumeroCSV(temp, "co_facturasrec340", "importepagado");

    nombreDato = util.translate("scripts", "Medio de pago utilizado");
    if ((contenidoRecibidas.length + 1) !=  371) {
      return this.iface.errorAcumuladoControl(371, nombreDato);
    }
    temp = qryFacturasRecibidas.value("mediopago");  		    
		temp = flfactppal.iface.pub_espaciosDerecha(temp, 1);
    contenidoRecibidas += temp;
    contenidoCSV += "|" + temp;

    nombreDato = util.translate("scripts", "Cuenta bancaria o medio de pago utilizado");
    if ((contenidoRecibidas.length + 1) !=  372) {
      return this.iface.errorAcumuladoControl(372, nombreDato);
    }
    temp = qryFacturasRecibidas.value("cuentabancaria");
    temp = flfactppal.iface.pub_espaciosDerecha(temp, 34);
    contenidoRecibidas += temp;
    contenidoCSV += "|" + temp;
    ...
    //Blancos del 406 al 500
    nombreDato = util.translate("scripts", "Espacios fin de registro recibidas");
    if ((contenidoRecibidas.length + 1) !=  406) {
      return this.iface.errorAcumuladoControl(406, nombreDato);
    }
...
function oficial_formatoNumeroSS340(numero, enteros, decimales)
{
  var resultado = "";

  resultado += flcontmode.iface.formatoNumeroSS(numero, enteros, decimales);
  return resultado;
}
//// OFICIAL /////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////
